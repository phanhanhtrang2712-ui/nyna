    ...
    >
      <Link to={`/san-pham/${item.id}`} className="relative mb-3 md:mb-6 rounded-[16px] md:rounded-[24px] overflow-hidden bg-gray-50 aspect-square shrink-0 block">
        {item.image ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-blue-50/40 p-4 text-center">
            <span className="text-[10px] font-black text-blue-900/30 uppercase tracking-widest mb-1.5">NYNA & SILA</span>
            <span className="text-[9px] font-bold text-gray-400 capitalize">Hình ảnh thật đang cập nhật</span>
          </div>
        )}
